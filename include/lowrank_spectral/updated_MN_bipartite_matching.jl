...
function bipartite_matching_setup(A::SparseMatrixCSC{T,Int64}) where T
    ...
    M_setup = MatrixNetworks.Matching_setup(rp,ci,ai,[],m,n)
    return M_setup
end
...
function bipartite_matching_primal_dual(rp::Vector{Int64}, ci::Vector{Int64}, 
                    ai::Vector{T}, m::Int64, n::Int64;normalize::Bool=true,tol::Float64=1e-8) where T
...
function bipartite_matching_primal_dual(M_setup::MatrixNetworks.Matching_setup;kwargs...)
    return bipartite_matching_primal_dual(M_setup.rp, M_setup.ci, M_setup.ai,
                                          M_setup.m, M_setup.n;kwargs...)
end
...
function bipartite_matching_local(A::SparseMatrixCSC{T,Int64};kwargs...) where T
    return bipartite_matching_primal_dual(bipartite_matching_setup(A);kwargs...)
end


function bipartite_matching_local(w::Vector{T},ei::Vector{Int64},
                                     ej::Vector{Int64},m::Int64,n::Int64) where T
...
edge_list(M_out)
"""
function edge_list_local(M_output::MatrixNetworks.Matching_output)
...
create_sparse(M_out)
"""
function create_sparse(M_output::MatrixNetworks.Matching_output)
    (ei,ej) = edge_list_local(M_output)
    A = sparse(ei,ej,1,M_output.m,M_output.n)
    return A
end

##########
function edge_indicator(M_output::MatrixNetworks.Matching_output, ei::Vector, ej::Vector)
