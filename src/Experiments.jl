        ...
        if profile 
            ...
            elseif method === ΛTAME_MultiMotif_M()
                ...
                if kwargs[:matchingMethod] === ΛTAME_GramMatching()
                    ...
                    A_motifCounts, B_motifCounts, perm, dup_vertices, (best_matching_score, max_motif_match, best_matched_motifs, best_matching) = fetch(future)
                else
                    A_motifCounts, B_motifCounts, perm,dup_vertices, (best_matching_score, max_motif_match, best_matched_motifs, _, _, best_matching) = fetch(future)
                end
                A_tris = -1
                B_tris = -1
            elseif method === EigenAlign_M() || method == Degree_M() || method === Random_M() || method === LowRankEigenAlign_M() || method === LowRankEigenAlignOnlyEdges_M()
