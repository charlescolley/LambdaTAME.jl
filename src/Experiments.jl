...
"""------------------------------------------------------------------------------
  ...
  found in the formatting\_specification.info file the data/ folder. The 
...
"""------------------------------------------------------------------------------
  ...
  align\_matrices or align\_tensors depending on the file extensions.

  Inputs
  ------
  * files - (Array{String,1}):
    List of the file names. Files may be either all SMAT or all STEN files. 
    align\_tensors or align\_matrices will be called depending on the extension. 
  * dirpath - (String):
...
"""------------------------------------------------------------------------------
  ...
  * seed\_exps - (Bool):
    ...
    number of p\_remove and n\_sizes variables, and the trial\_count. Changing 
    these may lead to different seeds. 
  * n\_sizes - (Array{Int,1}):
    The different sizes of graphs to generate experiments for. 
  * p\_remove - (Array{Float,1}):
...
------
  * exp\_results - (type omitted):
    The final array of results for the series of experiments, the size of each 
    entry will differ depending on whether or not profile was True. At the very
    least, the each entry will contain the seed used, the p\_remove value, the 
...
"""-----------------------------------------------------------------------------
  ...
  a graph is generated, we use the function ER\_noise\_model to generate a second 
  network to align against it. Once the second network is generated it is 
  randomly permuted. All additional kwargs are passed to the align\_matrices 
  function.
  ...
  * p\_edges - (func Int - Float):
    ...
    * All additional return variables are from align\_matrices function call. 
-----------------------------------------------------------------------------"""
...
"""------------------------------------------------------------------------------
  ...
  * method - (String):
  ...
  addition to whatever is returned by align\_tensors(\_profiled).
------------------------------------------------------------------------------"""
...
"""-----------------------------------------------------------------------------
  ...
  it's symmetrized. The bool use\_lcc is used to indicate whether or not to find
